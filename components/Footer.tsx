        ...
        <div className={`flex flex-col lg:flex-row justify-between items-start gap-20 mb-40 ${lang === 'ar' ? 'lg:flex-row-reverse' : ''}`}>
          <div className={`max-w-sm ${lang === 'ar' ? 'text-right' : 'text-left'}`}>
            <div 
              ...
              className={`flex items-center gap-3 mb-10 cursor-pointer group ${lang === 'ar' ? 'flex-row-reverse' : ''}`}
            >
              <div className="w-10 h-10 bg-blue-600 rounded-xl rotate-45 flex items-center justify-center transition-transform group-hover:rotate-90">
                <span className="text-white font-black text-xs -rotate-45 group-hover:rotate-[-90deg]">ZQ</span>
              </div>
              <span className="text-2xl font-bold text-white tracking-tighter uppercase">ZeroQueries</span>
            </div>
